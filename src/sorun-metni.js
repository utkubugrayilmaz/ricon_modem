...
const PROBLEM_TEXT_TR = {
  ...
  CONSOLE_NO_CREDENTIALS: { baslik: "Modem parolası tanımlı değil", neYap: "Bilgi işleme haber ver." },
  ...
  LOCK_STATE_UNKNOWN: { baslik: "SIM kilidi okunamadı",
    neYap: "PIN gönderilmedi. Tekrar dene; sürerse modemi kapat-aç." },
  PROFILE_UNKNOWN: { baslik: "Profil tanımlı değil", neYap: "Bilgi işleme haber ver." },
  ...
  INTERNET_DOWN: { baslik: "İnternet gelmedi", neYap: "Hattın açık ve kotalı olduğunu kontrol et." },
  AT_PORT_NOT_FOUND: { baslik: "SIM birimine ulaşılamadı", neYap: "Kapat-aç ve tekrar dene." },
  // --- Telefon numarasi ---
  ...
  MSISDN_NOT_ON_SIM: { baslik: "Numara SIM'de yok", neYap: "Elle gir." },
  MSISDN_MISMATCH: { baslik: "Girilen numara SIM'dekinden farklı",
  ...
  PIN_ATTEMPT_BURNED: { baslik: "Daha önce bir hak yanmış",
    neYap: "PIN'den emin olmadan deneme. Operatör kaydından doğrula." },
  PIN_REMAINING_UNKNOWN: { baslik: "Kalan hak okunamadı", neYap: "PIN'den emin ol." },
...
export function problemText(code) {
  ...
  return { code,
    ...
    neYap: `Bilgi işleme şu kodu bildir: ${code ?? "bilinmiyor"}` };
}
...
export function withProblemText(problems = []) {
