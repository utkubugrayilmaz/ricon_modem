...
import { konsolNvram, konsolYaz, konsolCalistir } from "./konsol.js";
import { LAN_IP_ANAHTARLARI } from "./profil.js";
...
export function provizyonPlanla(mevcut, profil) {
  const degisecek = {};
  const ayni = [];
  const eksik = [];
...
export function planiAyir(degisecek) {
  const lanIp = {};
  const digerleri = {};
  for (const [k, v] of Object.entries(degisecek)) {
    if (LAN_IP_ANAHTARLARI.includes(k)) lanIp[k] = v; else digerleri[k] = v;
  }
...
export async function provizyonUygula(opts, profil) {
  ...
  if (!kimlik) {
    rapor.problems.push(sorun("AUTH_REQUIRED", "telnet 5123"));
    rapor.ok = false;
    return rapor;
  }
  ...
  const { degerler, sayi, problems: okumaSorun } = await konsolNvram(kOpts);
  rapor.problems.push(...okumaSorun);
  if (sayi === 0) { rapor.ok = false; return rapor; }

  // 2) Planla
  const plan = provizyonPlanla(degerler, profil);
  const { lanIp, digerleri } = planiAyir(plan.degisecek);
  rapor.plan = {
  ...
  if (Object.keys(digerleri).length) {
    ...
    const y = await konsolYaz(kOpts, esle(digerleri));
  ...
  if (Object.keys(lanIp).length) {
    ...
    const y = await konsolYaz(kOpts, esle(lanIp));
    rapor.problems.push(...y.problems);
  ...
  if (opts.yeniHost || !Object.keys(lanIp).length) {
    ...
    rapor.ok = dog.tamam && sonucOk(rapor.problems);
  } else {
...
// degisecek {k:{mevcut,hedef}} -> konsolYaz icin {k:hedef}
...
async function rebootFireForget(kOpts) {
  try {
    await konsolCalistir({ ...kOpts, yazmaIzni: true, zamanAsimiMs: 4000 }, ["reboot"]);
...
async function dogrula(opts, profil, anaOpts) {
  ...
  for (let i = 0; i < maxDeneme; i += 1) {
    ...
    const { degerler, sayi } = await konsolNvram(kOpts);
