...
import {
  provisionModem, pcPreflight, readIdentity, telefonNormalize, settingLabel,
  SETTING_LABELS,
} from "./index.js";
...
export function createServer(opts = {}) {
  ...
  async function durumVer(yanit) {
    const on = pcPreflight(
      onek(fabrikaHost), onek(sahaHost),
    );
    ...
    jsonVer(yanit, 200, {
      ...
      modem: {
        konum: fabrikaVar ? "fabrika" : sahaVar ? "saha" : null,
        host: fabrikaVar ? fabrikaHost : sahaVar ? sahaHost : null,
      },
  ...
  async function hazirlaAkit(url, istek, yanit) {
    ...
    try {
      ...
      const konum = fabrikaVar
        ? { host: fabrikaHost, kaynakIp: on.fabrikaKaynak }
        : sahaVar ? { host: sahaHost, kaynakIp: on.sahaKaynak } : null;
      if (konum && kimlik) {
