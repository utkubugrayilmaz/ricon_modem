...
const KATALOG = {
  ...
  MSISDN_MISMATCH: (manual, device) => ({
    message: `The number typed by the operator (${manual}) differs from the one`
      + ` written on the SIM (${device}). The SIM's own value was used.`,
...
export function problem(code, ...args) {
  ...
  try {
    return { code, severity, ...KATALOG[code](...args) };
  } catch {
    return {
      code,
      severity: "error",
