...
const full = { modemPresent: true, simPresent: true, simLock: { lock: null },
...
test("modem yok -> once MODEM istenir (SIM/telefon sormanin anlami yok)", () => {
  const e = provisioningGaps({ ...full, modemPresent: false, simPresent: false });
...
test("PIN kilidi VAR ve PIN yok -> pin eksik", () => {
  const e = provisioningGaps({ ...full, simLock: { lock: "pin", pinRemaining: 3 } });
  assert.deepEqual(e, ["pin"]);
});

test("PIN kilidi var ama PIN VERILDI -> pin eksik DEGIL", () => {
  const e = provisioningGaps({ ...full, simLock: { lock: "pin" }, pin: "0270" });
  assert.deepEqual(e, []);
});

test("KILIT YOKSA pin HIC sorulmaz (proje hedefi PIN'siz akis)", () => {
  assert.deepEqual(provisioningGaps({ ...full, simLock: { lock: null } }), []);
  assert.deepEqual(provisioningGaps({ ...full, simLock: null }), []);
});

test("PUK kilidi 'eksik girdi' DEGIL — insan mudahalesi, problems ile bildirilir", () => {
  const e = provisioningGaps({ ...full, simLock: { lock: "puk", pukRemaining: 9 } });
  assert.deepEqual(e, [], "PUK icin PIN sorulmaz");
});

test("cok eksik varsa hepsi listelenir, sira en temelden", () => {
  const e = provisioningGaps({ modemPresent: true, simPresent: true,
    simLock: { lock: "pin" }, phone: null, pin: null });
