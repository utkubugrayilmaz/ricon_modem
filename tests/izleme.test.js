...
import { findOutages, systemView } from "../src/index.js";

// Ornek: erisim var ama internet yok -> "internet" kesintisi;
// erisim yok -> "yonetim" kesintisi (reboot penceresi).
const o = (an_sn, reachable, internet) => ({ an_sn, reachable, internet });
...
test("kesintileriBul: internet kesintisi baslangic/bitis/sure ile bulunur", () => {
  ...
  assert.deepEqual(k[0], { kind: "internet", basla_sn: 0, bitis_sn: 10,
    durationSec: 10, hala_suruyor: false });
});
...
test("kesintileriBul: yonetim kesintisi (cihaz cevap vermiyor) AYRI raporlanir", () => {
  ...
  const yonetim = k.filter((x) => x.kind === "yonetim");
  assert.equal(yonetim.length, 1);
  assert.equal(yonetim[0].basla_sn, 5);
  assert.equal(yonetim[0].durationSec, 10);
  ...
  assert.equal(k.filter((x) => x.kind === "internet").length, 0);
});

test("kesintileriBul: sonuna kadar suren kesinti hala_suruyor:true", () => {
  const k = findOutages([o(0, true, false), o(5, true, false)]);
  assert.equal(k[0].hala_suruyor, true);
  assert.equal(k[0].durationSec, 5);
});

test("kesintileriBul: kesinti yoksa bos dizi", () => {
  assert.deepEqual(findOutages([o(0, true, true), o(5, true, true)]), []);
  assert.deepEqual(findOutages([]), []);
});
...
test("systemView: ham alanlardan okunabilir gorunum, bos alan undefined", () => {
  const g = systemView({
    lan_ip: "5.5.5.1", lanMac: "00:0C:43:43:5F:4E", wl_radio: "0",  // CIHAZIN anahtarlari
    uptime_spe: "12:00 up 3 Min.", lan_proto: "static", mem_info: "  ",
  });
  assert.equal(g.lan_ip, "5.5.5.1");
  assert.equal(g.lanMacVendor, "Ralink/MediaTek", "OUI'den uretici turetiliyor");
  assert.equal(g.uptime, "12:00 up 3 Min.");
  assert.equal(g.memory, undefined, "sadece bosluk olan alan undefined");
