...
test("summarizeMetrics: comparison iki AYRI iddia uretir", () => {
  const o = summarizeMetrics(Array.from({ length: 6 }, () => line()),
    { manualSec: 900, manualSource: "3 olcum", manualCount: 3, modemCount: 100 });
  // cycle = giris(10) + arac(40) = 50 sn; 900 -> 50 = %94.4, 18x
  assert.equal(o.cycleSec, 50);
  assert.equal(o.comparison.cycle.reductionPct, 94.4);
  assert.equal(o.comparison.cycle.speedup, 18);
  // insan mesgul = 10 sn; 900 -> 10 = %98.9
  assert.equal(o.comparison.humanBusy.reductionPct, 98.9);
  assert.equal(o.comparison.scale.savedHours, 23.6);
  assert.equal(o.comparison.uyari, undefined, "6 calistirma yeterli, uyari yok");
});
...
test("summarizeMetrics: az ornek ve beyan tabani UYARI uretir", () => {
  ...
  assert.match(o.comparison.uyari, /en az 5/);
